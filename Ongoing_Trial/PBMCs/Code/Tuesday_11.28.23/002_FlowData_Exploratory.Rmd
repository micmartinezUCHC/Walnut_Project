...
setwd("/Users/mikemartinez/Desktop/Walnut_Project/PBMCs/Exploratory_Analysis/")
...
lymphocyte_folder <- "/users/mikemartinez/Desktop/Walnut_Project/PBMCs/Data/Reformatted_Data/Batch2_3_Lymphocytes/"
monocyte_folder <- "/users/mikemartinez/Desktop/Walnut_Project/PBMCs/Data/Reformatted_Data/Batch2_3_Monocytes/"
...
for (result in names(lymphocyte_results)) {
  ...
  shapiro_test_result <- shapiro.test(lymphocyte_results[[result]]$X.Cells)
  print(result)
...
for (result in names(monocyte_results)) {
  ...
  # Run the Shapiro test
  shapiro_test_result <- shapiro.test(monocyte_results[[result]]$X.Cells)
  print(result)
  print(shapiro_test_result)
...
for (result in names(lymphocyte_results)) {
  ...
  hist(lymphocyte_results[[result]]$X.Cells, main = title, breaks = 10, xlab = "Number of Cells")
  dev.off()
}
...
for (result in names(monocyte_results)) {
  ...
  png(paste(title, "histogram.png", sep = "_"))
  # Plot histogram of cells
  hist(monocyte_results[[result]]$X.Cells, main = title, breaks = 10, xlab = "Number of Cells")
  dev.off()
}
```
...
for (result in names(lymphocyte_results)) {
  ...
  # Plot
  pairedPlot <- ggpaired(lymphocyte_results[[result]], x = "Visit", y = "X.Cells", id = "Sample_Number",
  ...
  stat_compare_means(method = "wilcox", paired = TRUE, vjust = 1.2)
...
for (result in names(monocyte_results)) {
  ...
  # Plot
  pairedPlot <- ggpaired(monocyte_results[[result]], x = "Visit", y = "X.Cells", id = "Sample_Number",
                  fill = "Visit", line.color = "black", line.size = 0.3) +
  labs(y = "Number of Cells",
       x = "Visit") +
  theme(legend.position = "none") +
